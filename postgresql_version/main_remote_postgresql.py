...
from SignInWidget_remote_postgresql import SignInWidget
...
class MainWindow(QMainWindow, Ui_MainWindow):
    ...
    def collect_info(self, id, name):
        ...
        cur = db_con.cursor()
        res = cur.execute(f"""SELECT name from main.halls
                            WHERE parent_theatre=(SELECT id from main.theatres
                                                WHERE theatres.name like '{self.theatre_name}')""")
        res = cur.fetchall()
    ...
    def updatehall(self):
        ...
        cur = db_con.cursor()
        cur.execute(f"""SELECT date, "film name", "start time", duration,
                                "free sits", "ticket price", id from main.sessions
                            WHERE "hall id"=(SELECT id from main.halls
                                            WHERE name like '{self.current_hall_name}')""")
        res = cur.fetchall()
    ...
    def update_all_sessions_tb(self):
        cur = db_con.cursor()
        cur.execute(f"""SELECT date, "film name", "start time", duration,
                                    "hall id", "free sits", "ticket price", id from main.sessions
        ...
        for i in range(len(res)):
            for j in range(ALL_COLUMN_COUNT):
                if j == 4:
                    cur.execute(f"""SELECT name from main.halls
                                            WHERE id={res[i][j]}""")
                    hall_name = cur.fetchone()[0]
...
class HallDialog(QDialog):
    ...
    def confirm(self):
        cur = db_con.cursor()
        cur.execute(f"""SELECT name from main.halls
                             WHERE parent_theatre={self.theatre_id}""")
        res = cur.fetchall()
        names = [str(x[0]) for x in res]
        ...
        if self.mode == ADD:
            ...
            if self.name in names:
                ...
                cur.execute(f"""INSERT INTO main.halls(name, parent_theatre, width, height)
...
class SessionDialog(QDialog, Ui_NewSessionDialog):
    ...
    def update_session(self):
        self.collect_info()
        cur = db_con.cursor()
        cur.execute(f"""UPDATE main.sessions
                        SET date='{self.date}',
    ...
    def add_session(self):
        ...
        if not self.film_name:
            ...
            cur = db_con.cursor()
            cur.execute(f"""SELECT id from main.halls WHERE name like '{self.hall_name}'""")
            hall_id = cur.fetchone()[0]
            cur.execute(f"""SELECT height, width from main.halls WHERE id={hall_id}""")
            free_sits = cur.fetchall()[0]
            free_sits = free_sits[0] * free_sits[1]
            cur.execute(f"""INSERT INTO main.sessions(date, "film name", "start time", duration,
...
class SellTicket(QWidget, Ui_SellWidget):
    def __init__(self, items):
        ...
        cur = db_con.cursor()
        cur.execute(f"""SELECT id from main.halls
                                        WHERE name like '{self.hall_name}'""")
        self.hall_id = cur.fetchone()[0]

        cur.execute(f"""SELECT width, height from main.halls
                                        WHERE id={self.hall_id}""")
        column_count, row_count = cur.fetchone()
        self.seats_table.setColumnCount(column_count)
        self.seats_table.setRowCount(row_count)
        cur.execute(f"""SELECT "row", "column" from main.booked_seats
                                                WHERE session_id={self.session_id}""")
        self.booked_seats = cur.fetchall()
        print('asda')
    ...
    def sell_tickets(self):
        ...
        if self.numbers:
            for row, col in self.numbers:
                cur.execute(f"""INSERT INTO main.booked_seats (session_id, "row", "column")
        ...
        cur.execute(f"""UPDATE main.sessions
                        SET "free sits"=(SELECT width from main.halls WHERE id={self.hall_id}) *
                        (SELECT height from main.halls WHERE id={self.hall_id}) - {len(self.booked_seats)}
                        WHERE id={self.session_id}""")
        db_con.commit()
        self.close()
        main_window.updatehall()
